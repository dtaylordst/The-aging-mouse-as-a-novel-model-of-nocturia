...
for(i=0; i<list.length; i++) {
 ...
 if(nImages>=1) {
  if(i==0) {
	...
	run("Set Measurements...", "area centroid shape limit redirect=None decimal=3");
	run("Measure");
	run("Crop");
	run("Clear Outside");
	run("8-bit");	
	...
	run("Measure");	
	run("Analyze Particles...", "size=16-Infinity pixel circularity=0.001-1.00 show=Outlines display include");
